...
// config/examples/delta directory from
// https://github.com/MarlinFirmware/Configurations/branches/all and customize
// for your machine.
//
...
// config/examples/SCARA directory from
// https://github.com/MarlinFirmware/Configurations/branches/all and customize
// for your machine.
//
...
#define MPE_FAST_SPEED                                                         \
  3000 // (mm/min) Speed for travel before last distance point
#define MPE_SLOW_SPEED                                                         \
  2000 // (mm/min) Speed for last distance travel to park and couple
...
// distance from the extruder 0 hotend. #define HOTEND_OFFSET_X { 0.0, 373.3 }
// // (mm) relative X-offset for each nozzle
#define HOTEND_OFFSET_Y                                                        \
  { 0.0, 2 } // (mm) relative Y-offset for each nozzle
#define HOTEND_OFFSET_Z                                                        \
  { 0.0, 0.00 } // (mm) relative Z-offset for each nozzle
...
// if the chamber is over this temperature
...
#define TEMP_RESIDENCY_TIME                                                    \
  2 // (seconds) Time to wait for hotend to "settle" in M109
#define TEMP_WINDOW                                                            \
...
//(useful for mismatched extruders)                               // Set/get
// with gcode: M301 E[extruder number, 0-2]
...
#define PID_FUNCTIONAL_RANGE                                                   \
  35 // If the temperature difference between the target temperature and the
...
#define MAX_ACCEL_EDIT_VALUES                                                  \
  { 8000, 8000, 600, 20000 } // ...or, set your own edit limits
#endif
...
#define Z_PROBE_LOW_POINT                                                      \
  -25 // Farthest distance below the trigger-point to go before stopping
...
// Also enable NO_MOTION_BEFORE_HOMING for extra safety. #define
// UNKNOWN_Z_NO_RAISE      // Don't raise Z (lower the bed) if Z is "unknown."
...
#define DEFAULT_KEEPALIVE_INTERVAL                                             \
  5 // Number of seconds between "busy" messages. Set with M113.
...
// Backlight #define FYSETC_GENERIC_12864_1_1 // Larger display with basic ON/OFF
// backlight.
